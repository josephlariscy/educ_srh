...
F.model.srh <- lm(health.rev ~ F.year.f + F.age.mean.cen, data = F.regression)
summary(F.model.srh)
...
F.model.educ <- lm(educ ~ F.year.f + F.age.mean.cen, data = F.regression)
...
# Predict mean SRH for each year for women, age adjusted
M.model.srh <- lm(health.rev ~ M.year.f + M.age.mean.cen, data = M.regression)
summary(M.model.srh)
...
M.model.educ <- lm(educ ~ M.year.f + M.age.mean.cen, data = M.regression)
...
p1 <- ggplot(data = F.educ.srh) +
  ...
  geom_line(aes(y = F.Z.educ, x = year), size = 1.05) +
  geom_line(aes(y = F.Z.srh,  x = year), size = 1.05, color = "grey") +
  geom_point(aes(y = F.Z.educ, x = year), shape = 18, size=3) +
  geom_point(aes(y = F.Z.srh,  x = year), shape = 15, size=3, color="grey") +
  coord_cartesian(ylim = c(-3, 3)) +
  scale_y_continuous(breaks = seq(-3, 3, by = 1), labels = label_true_minus) +
  scale_x_continuous(breaks = seq(1972, 2018, by = 2)) +
  ...
  geom_segment(x = 1996.5, xend = 2002, y = -3, yend = -3,
               arrow = arrow(ends = "last", angle = 90, length=unit(.2,"cm"))) +
  annotate("text", y =-3, x = 1987, size = 3.5,
...
p2 <- ggplot(data = M.educ.srh) +
  geom_vline(xintercept = 2002.5, linetype = "dashed") +
  geom_line(aes(y = M.Z.educ, x = year), size = 1.05) +
  geom_line(aes(y = M.Z.srh,  x = year), size = 1.05, color = "grey") +
  geom_point(aes(y = M.Z.educ, x = year), shape = 18, size=3) +
  geom_point(aes(y = M.Z.srh,  x = year), shape = 15, size=3, color="grey") +
  coord_cartesian(ylim = c(-3, 3)) +
  scale_y_continuous(breaks = seq(-3, 3, by = 1), labels = label_true_minus) +
  scale_x_continuous(breaks = seq(1972, 2018, by = 2)) +
  ylab(expression(bold('Standardized metric units'))) +
...
ggsave("C:/Users/jtlriscy/educ_srh/figure.png")
