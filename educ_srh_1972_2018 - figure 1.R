...
# Gender-Specific Trends in Educational Attainment and Self-Rated Health
   # June 9, 2023: I removed weights

# Run program educ_srh_1972_2018.R before running this program
...
p1 <- ggplot(data = F.educ.srh) +
  ...
  geom_line(aes(y = F.Z.educ, x = year), size = 1.05, color = "#500000") +
  geom_point(aes(y = F.Z.srh,  x = year), shape = 15, color = "#898D8D", size = 3) +
  geom_point(aes(y = F.Z.educ, x = year), shape = 18, color = "#500000", size = 3) +
  coord_cartesian(ylim = c(-3, 3)) +
  scale_y_continuous(breaks = seq(-3, 3, by = 1), labels = label_true_minus) +
  scale_x_continuous(breaks = seq(1972, 2018, by = 2)) +
...
p2 <- ggplot(data = M.educ.srh) +
  ...
  geom_line(aes(y = M.Z.educ, x = year), size = 1.05, color = "#500000") +
  geom_point(aes(y = M.Z.srh,  x = year), shape = 15, color = "#898D8D", size = 3) +
  geom_point(aes(y = M.Z.educ, x = year), shape = 18, color = "#500000", size = 3) +
  coord_cartesian(ylim = c(-3, 3)) +
  scale_y_continuous(breaks = seq(-3, 3, by = 1), labels = label_true_minus) +
  scale_x_continuous(breaks = seq(1972, 2018, by = 2)) +
  ylab(expression(bold('Standardized units'))) +
  ...
  annotate("text", y =  2, x = 2014, size = 14/.pt, label = "Education", 
           family = "Times New Roman") +
  annotate("text", y = -2, x = 2008, size = 14/.pt, label = "Self-Rated Health", 
           family = "Times New Roman") 
...
ggsave("C:/Users/jlariscy/educ_srh/analysis/figure1.png", bg = "white")
